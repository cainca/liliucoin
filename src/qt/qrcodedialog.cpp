...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("DNG")
    updateDisplayUnit();
}
...
{
    QString ret = QString("liliucoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non DNG unit input in lnReqAmount, we generate the URI with DNG as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::DNG, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
