...
def check_json_precision():
    """Make sure json library being used does not lose precision converting DNG values"""
...
def connect_JSON(config):
    ...
    if not 'rpcport' in config:
        config['rpcport'] = 13034 if testnet else 3034
...
def create_tx(dongcoind, fromaddresses, toaddress, amount, fee):
    ...
    if total_available < needed:
        sys.stderr.write("Error, only %f DNG available, need %f\n"%(total_available, needed));
        sys.exit(1)
